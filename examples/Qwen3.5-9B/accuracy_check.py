"""
Accuracy checker: verify that the custom Qwen3.5-9B implementation
...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=0 .venv/bin/python accuracy_checker.py --model Qwen/Qwen3.5-9B

The agent's serving module is expected to export a model-loading function
that returns a model whose `__call__(input_ids, past_key_values)` yields
`(logits, past_key_values)` matching HF eager numerics. We probe several
common names so the agent has flexibility in naming.
"""
...
def _resolve_load_fn():
    """Find the agent's model loader. Tries several conventional names."""
    candidates = [
        ("main", "load_qwen_model"),
        ("main", "load_model"),
        ("step_1_fastapi", "load_qwen_model"),
        ("step_1_fastapi", "load_model"),
        ("step_1_fastapi", "load_llama_model"),  # legacy
    ]
    for mod_name, fn_name in candidates:
        try:
            ...
            continue
    raise ImportError(
        "Could not find model loader in any of: "
        + ", ".join(f"{m}.{f}" for m, f in candidates)
    )


_load_custom_model = _resolve_load_fn()
...
def main():
    ...
    t0 = time.perf_counter()
    custom_model, _ = _load_custom_model(args.model, args.device, dtype, hf_token)
